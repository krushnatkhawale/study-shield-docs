...
| [SS-EXP-03](experience.md#ss-exp-03-app-speaks-the-parents-language) | Hindi/Marathi app chrome: `values-hi/` + `values-mr/` on the first-quiz path (auth, add child, find TV, start, results); first-run language picker before the stepper persists `SessionManager.appLocale` and applies via `AppCompatDelegate.setApplicationLocales` | TV greeting stays the per-kid `greetingLanguage`; English fallback for untranslated strings |
